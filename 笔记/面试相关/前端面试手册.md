...
```
！important > 行内样式 > id选择器 > 属性/类/伪类选择器 > 标签选择器 > 通配符选择器*
```
...
```
答：流式布局(百分比布局） rem布局(配合媒体查询使用)  弹性布局flex
```
...
6. 3D转换 perspective(透视距)  transform-style(3D空间效果)
...
```
答：重排：当DOM元素影响了元素的几何属性(比如宽和高)，浏览器需要重新计算元素的几何属性，其他元素的几何属性和位置会因此受到影响，浏览器会使渲染树中受影响的部分失效，并重新规则渲染树，这个过程称为重排。
   重绘：完成重排后，浏览器会重新绘制受影响的部分到屏幕上，该过程称为重绘。
   当我们改变DOM的大小，增加删除都会导致重排，当给DOM元素改变颜色的时候，会导致重绘，重排一定会重绘，重绘不一会重排。重排会影响性能，所以我们尽快能的减少重排的操作。
```

## 18. 为什么要进行 css 的初始化

```
答：1.浏览器差异：不同浏览器对有些标签的默认值是不同的，如果没对css初始化会出现浏览器之间页面显示差异
    2.提高编码质量：如果不初始化，重复的css样式很多
```
...
```
答：原因 1. 浮动导致的塌陷，浮动的盒子会调理标准流
        2. 嵌套的两个盒子，子盒子设置margin-top，会导致父盒子一起下移
    解决方式：
    第一种情况：1.清除浮动
              2.给父盒子加高度
...
## 0. const声明的数组和对象可以改变吗

可以 因为他们是引用类型 是复杂数据类型。也就是他们在内存地址中所储存的指针没有改变 所以可以。
相反简单数据类型就不可以
...
```
答：Promise是异步编程的一种解决方案,它有三种状态
      1. pending 等待(进行中)
      2. fulfilled 成功(已完成) 调用resolve,promise的状态会被标记为成功(fulfilled)
      3. rejected 失败(拒绝) 调用reject,promise的状态会被标记为失败(rejected)
    promise的状态只能修改一次,一旦状态被修改就会状态凝固,所以多次调用resolve和reject是没有意义的
    作用:
      1. promise可以解决回调嵌套问题(回调函数阅读性不好,可维护性差)
...
1. 浅拷贝：拷贝对象的一层属性，如果对象里面还有对象，拷贝的是地址，之间修改会有影响，适用于对象里面属性的值是简单数据类型的。
...
2. 深拷贝：拷贝对象的多层属性，如果对象里面还有对象，会继续拷贝，之间修改不会有影响，使用递归去实现。
...
```
答：use strict 是ES5新增的指令，俗称js的“严格模式”。
    "严格模式"是一种在JavaScript代码运行时自动实行更严格解析和错误处理的方法。这种模式使得Javascript在更严格的条件下运行。
好处：1.消除JavaScript语法的一些不合理、不严谨之处，减少一些怪异行为
     2.消除代码运行的一些不安全之处，保证代码运行的安全
     3.提高编译器效率，增加运行速度
     4.为未来新版本的JavaScript做好铺垫
坏处：现在网站的JS都会进行压缩，一些文件用了严格模式，而另一些没有，这时这些本来是严格模式的文件被合并后后，这个串就到文件的中间，不仅没有指示严格模式，反而在压缩后浪费了字节。
```

## 20. js 的数据类型有哪些，内置对象有哪些

```
答：简单数据类型：number string boolean null undefined symbol(ES6新增) BigInt
   复杂数据类型：Object Data function Array
   内置对象：Math Date Object Array Function Number String...
```

## 21. js 的逻辑判断是什么

```
答： &&：左右两边的条件都成立，结果为true，只要有一个不成立，结果为false从左往右，（找假值(布尔类型为false的值)，找到了逻辑就中断了，代码短路了，后续代码就不执行了，返回找到的真值。如果没有找到真值，就返回最后一个假值）
    ||：左右条件只要有一个成立，结果为true，两个都不成立，结果才为false（从左往右，找真值(布尔类型为true的值)，找到了逻辑就中断了，代码短路了，后续代码就不执行了，返回找到的假值。如果没有找到假值，就返回最后一个真值）
```
...
答：1.call和apply方法都可以调用函数，方法内的第一个参数可以修改this的指向
   2.call方法可以有多个参数，除了第一个参数，其他参数作为实参传递给函数
      apply方法最多只有2个参数，第二个参数是数组或伪数组，数组里面的每一项作为实参传递给函数
   3.bind方法不能调用函数，它会返回一个函数，要再次调用才能执行。并且绑定新函数的this为bind方法的第一个参数。
```

## 23. es5 新增的数组的方法

```
答：forEach() 循环遍历数组，功能等同于for
   some()    遍历数组，如果至少有一个满足条件，就返回true，否则返回false
   map()     返回一个新数组，该数组中的每个元素是调用一个提供的函数后返回的结果
   filter()  用于过滤掉不满足条件的元素，返回一个满足条件的所有元素的新数组
   every()   遍历数组，只有当所有元素返回true，才返回true，封装返回false
   reduce()  对数组中的每个元素执行一个提供函数，将其结果汇总，返回出去
```

## 24. for in 和 for of 区别

```
答：for-in 循环返回的值都是数据结构的键值名。遍历对象返回的对象的key值,遍历数组返回的数组的下标           for-in更适合遍历对象
   for-of是ES6标准，循环用来获取一对键值对中的值(value),而 for-in获取的是键名。for-of更适合遍历数组
```
...
5、箭头函数
6、proxy代理
7、promise（axios基于promise）
8、async和await 
9、flat数组降维
10、Object.assign 合并对象
11、import 引入
12、export 导出
```
...
```
答：防抖 指事件触发后在n秒内函数只能执行一次，如果在n秒内又触发了事件，则会重新计算函数执行的时间。
    例如：设定1000毫秒执行，当你触发事件了，它会1000毫秒后执行，但是在还剩500毫秒的时候又触发了事件，那就会重新开始1000毫秒之后再执行。

    节流：指连续触发事件但是在设定的一段时间内只执行一次函数
...
```
答：设计模式的原则："找出 程序中变化的地方，并将变化封装起来"
    1、单例模式,也就是对象字面量的方式来创建对象，然后添加属性和方法
    2、工厂模式,通常是写一个方法，然后在方法内来创建实例
    3、构造函数模式,直接将属性和方法赋给this，通过new进行调用构造函数
    4、原型模式,将属性和方法添加到原型对象上，实例可以共享
   
```

## 28. js 中如何检测一个变量是 string 类型
...
## 29. Json 字符串和 json 对象怎么相互转换

```
答：Json字符串Json对象：Json.parse(Json字符串)
    Json对象Json字符串：Json.stringify(Json对象)
```
...
10. splice()
    ...
    index:　　要操作的位置，　　　　     正数　　　　/　　负数，　　　　表示：从前往后　　/　　从后往前
    num：　　　　  0　　　　　　　　　　　　　　　　　　　　/　　其他数字，　　表示：不删除，要增加，这时一般后面会有item　　/　　删除的个数）item：　　num为0时，要添加的元素
    ```

11. slice(start, end);  选择数组，选取数组组中[start, end] 的元素 ， 原数组不改变复制数组的数据，参数1从哪开始，参数2到哪结束
...
13.**trim()  只能去除字符串前后的空白**

14.**字符串.toUpperCase() 字符串转成大写**
...
```js
答：1.https需要到CA申请证书，一般免费证书较少，因而需要一定的费用。
    2.http是超文本传输协议，信息是明文传输，https是具有安全性的ssl/tls加密传输协议
    3.http和https连接方式不同，端口也不一样，http端口是80，https是443.
    4.http的连接很简单，是无状态的，https需要是由SSL/TLS+http协议构建的可进行加密传输、身份认证的网络协议，比http协议安全。
```

## 13. 一个页面从输入 url 到页面加载显示完成，这个过程发生了什么

```
答：1.输入url后，先进行DNS解析，将相应的域名解析为IP地址
    2.根据IP地址寻找相应的服务器
    3.与服务端进行TCP的三次握手，建立连接
    4.客户端发送请求，找到相应的资源库
    5.客户端接收数据，进行相应的渲染
```

## 14. 浏览器为什么会有缓存，怎么清除缓存

```
答：原因: 当我们请求的地址相同时，浏览器为了提高性能，会把相同地址的数据进行缓存，如果服务端的数据发生改变，客户端也不会更新，这就需要清除缓存。
    解决：在url后面加上时间戳
    例：https：//www.baidui.com?time = new Date()
```
...
```js
答：阻止事件冒泡和捕获 e.stopPropagation()
    阻止事件默认行为  e.preventDefault()   return  false
    注意：在高版本浏览器中，使用addEventListener注册的事件，return false不起效果，必须使用事件对象
```

###

## 11. 原生注册事件的方式有哪些？区别是什么
...
答：方式
    ...
    2. addEventListener
    区别：
    1. on注册事件，同一元素只能注册一次同一类型事件，否则会被覆盖
    2. addEventListener可以注册多个同一类型的事件，不会被覆盖
```
...
答：1. git add .
    2. git commit -m '.........'
    3. git pull --rebase #获取最新代码 (如果有冲突，继续一下步骤，如果无 直接4. git push)
    5. git status （查看冲突的文件）
    6. git add （修改冲突文件后，添加文件到缓存区）
...
```
答：基本概念：vue-loader是基于webpack的一个的loader，解析和转换.vue文件。提取出其中的逻辑代码 script,样式代码style,以及HTML 模板template，再分别把他们交给对应的loader去处理。
    用途：js可以写es6、style样式可以scss或less等
```

## 10. webpack 与 gulp 的不同？

```
答：gulp 是基于任务和流的，找到一个或一类的文件，对其进行一系列链式操作，整个链式操作构成一个任务，多个任务构成web整个构建流程。
  webpack 是基于入口的，自动地递归解析所有依赖的资源文件，用不同的loader来处理不同的文件，用plugin来扩展webpack的功能。
```

## 11.webpack 的构建流程是什么（打包的原理）

```
答：Webpack 的运行流程是一个串行的过程,从启动到结束会依次执行以下流程 :
...
## 12.webpack 如何处理 es6 及以上的兼容问题

```
答：使用 babel-loader @babel/core @babel/preset-env 插件
```

## 13.webpack 中 loader 和 plugin 的区别
...
答：loader
    ...
    区别：
    1. 对于loader，它是一个转换器，将A文件进行编译形成B文件，这里操作的是文件，比如将A.scss转换为A.css，单纯的文件转换过程。
...
```
npm和yarn
平时用yarn下载一些依赖包 很方便。跟npm比起来 感觉yarn的下载速度更快一点 还有界面简洁，
最重要的是yarn下载的依赖包版本都是一样的，yarn可以进行离线下载
```
...
```
    ...
    view传送指令到controller,controller完成业务逻辑后,要求model改变状态, model将新的数据发送到view,用户得到反馈,所有的通信都是单向的。
```
...
答：1. 遵循规范
    ...
    2. 加载的时间
       ...
       import：编译时加载（效率更高），所以import命令会提升到整个模块的头部。
    3. 本质
...
```
答：AMD 和 CMD 都是js模块化的规范
    区别：
    1. 对于依赖的模块，AMD 是提前执行，CMD 是延迟执行。
    2. CMD 推崇依赖就近，只有在用到某个模块的时候再去require
       AMD 推崇依赖前置，在定义模块的时候就要声明其依赖的模块
    3. RequireJS 遵循 AMD（异步模块定义）规范，Sea.js 遵循 CMD （通用模块定义）规范
```

## 19. v-if 和 v-show 的区别，适合场景

```
答：v-if 是通过创建和删除DOM元素来控制元素的显示和隐藏, 适用于不需要频繁切换元素显示和隐藏的情况。
   v-show 通过操作css的display: block和display:none来控制元素的显示和隐藏,适用于需要频繁切换元素的显示和隐藏的情况。
```

## 20. v-for 和 v-if 的优先级那个更高

```
答：v-for的优先级要高于v-if
    如果同时出现v-for和v-if,无论判断条件是否成立,都会执行一遍v-for循环遍历,这样浪费性能,要尽可能的避免两者一起使用。
```

## 21. vue 中的事件修饰符有哪些，分别作用是什么
...
## 22. v-model 的修饰符有哪些，分别作用是什么

```
答：.trim 去除首尾空格
    .lazy 只在输入框失去焦点或者按回车键时更新内容,不是实时更新
...
## 25. vue 中怎么给 data 动态添加数据，为什么要这样写

```js
答: 1、官方文档定义：如果在实例创建之后添加新的属性到实例上，它不会触发视图更新.
    2、原因：受现代JavaScript的限制 ，Vue不能检测到对象属性的添加或删除。由于Vue会在初始化实例时对属性执行 getter/setter转化过程（使用Object.defineProperty进行数据的劫持），所以属性必须在data对象上存在才能让Vue转换它，这样才能让它是响应的。
    方法: 1、this.$set(对象名,属性,值) 或 Vue.set(对象名,属性,值)  添加单个属性
...
## 26. \$nextTick 的原理和使用的场景

```
答：1.使用原理:
      1）Vue是异步执行dom更新的，一旦观察到数据变化，Vue就会开启一个队列，然后把在同一个事件循环当中观察到数据变化的 watcher 推送进这个队列。如果这个watcher被触发多次，只会被推送到队列一次。这种缓冲行为可以有效的去掉重复数据造成的不必要的计算和DOm操作，这样可以提高渲染效率。
      2）如果要获取更新后的DOM元素，可以使用vue内置的$nextTick方法，参数是一个函数。它的作用类似setTimeout，进行执行异步的操作。
    2.应用：vue中的nextTick主要用于处理数据动态变化后，DOM还未及时更新的问题，用nextTick就可以获取数据更新后最新DOM的变化。
    3.场景：
...
```
答： vue组件是可复用的vue实例, 组件中的data写成一个函数,并且以函数返回值的形式定义,函数形成局部作用域，这样每次复用组件,就会返回一个新的data,确保data数据的独立性,各个组件之间修改数据不会相互影响.如果data使用对象，对象是引用类型。会导致复用组件中的data都指向了同一块内存，那么各组件的数据就会相互影响，不能保持独立性了。
```

## 28. 组件化和模块化的区别

```
答: 组件化: 主要是界面上的划分,是一个对于结构样式行为的整体拆分,方便调用和复用
    模块化: 主要从代码逻辑的角度进行划分,对js功能的拆分和封装,方便代码分层开发,保证每个功能模块职责单一
    组件化模块化优点：开发调试效率高、可维护性强、避免阻断、版本管理更容易
```

## 29. 怎么理解 vue 中的虚拟 DOM

```
答：虚拟DOM其实就是一棵以JavaScript对象( VNode 节点)作为基础的树，用对象属性来描述节点，实际上它只是一层对真实 DOM 的抽象。最终可以通过一系列操作使这棵树映射到真实环境上。简单理解可以把Virtual DOM  理解为一个简单的JS对象，并且最少包含标签名( tag)、属性(attrs)和子元素对象( children)三个属性。不同的框架对这三个属性的命名会有点差别。vue的虚拟dom是将多次dom操作保存在一个js对象（虚拟dom对象）中，然后用这个js对象一次性的去更新dom操作，这样就避免了很多无效的计算。
```

## 30. 全局前置导航守卫

router.beforeEach((to, from, next) => {  // ... })
添加了这个之后 用户就不能随意进入其他页面 除了 login以外 在这里判断用户的token值是否合法
